...
import { z } from 'zod';
import { RequestSchema, ProgressTokenSchema } from '@modelcontextprotocol/sdk/types.js';
...
export class MCPServer {
  ...
  private createTransport(): BaseTransport {
    ...
    switch (this.transportConfig.type) {
      ...
      case 'http-stream': {
        ...
        logger.debug(
          `Creating HttpStreamTransport with effective responseMode: ${httpConfig.responseMode}`
        );
  ...
  private setupHandlers() {
    ...
    // TODO: Replace 'any' with the specific inferred request type from the SDK schema if available
    this.server.setRequestHandler(CallToolRequestSchema, async (request: any, extra?) => {
